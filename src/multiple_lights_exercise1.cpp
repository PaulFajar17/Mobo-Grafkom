...
int environment = 1; // 1 = Desert, 2 = Factory, 3 = Horror, 4 = Biochemical Lab
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Multiple Lights Exercise 1 (Atmospheres)", NULL, NULL);
    if (window == NULL)
    {
    ...
    // Default variables for the 4 environments
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Atmosphere logic based on 'environment' variable
        glm::vec3 clearColor;
        ...
        if (environment == 1) {
            ...
            for(int i=0; i<4; i++){
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].ambient", pointLightColors[i] * 0.1f);
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].diffuse", pointLightColors[i]);
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].specular", pointLightColors[i]);
            }
        } 
            ...
            pointLightColors[3] = glm::vec3(0.3f, 0.1f, 0.1f); // One dim red light
            for(int i=0; i<4; i++){
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].ambient", pointLightColors[i] * 0.1f);
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].diffuse", pointLightColors[i]);
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].specular", pointLightColors[i]);
            ...
            // Biochemical Lab
            clearColor = glm::vec3(0.1f, 0.2f, 0.1f);
            glClearColor(clearColor.r, clearColor.g, clearColor.b, 1.0f);
            
            lightingShader.use();
            // directional light
            lightingShader.setVec3("dirLight.direction", -0.2f, -1.0f, -0.3f);
            lightingShader.setVec3("dirLight.ambient", 0.05f, 0.1f, 0.05f);
            lightingShader.setVec3("dirLight.diffuse", 0.1f, 0.3f, 0.1f);
            lightingShader.setVec3("dirLight.specular", 0.2f, 0.6f, 0.2f);
            // point light config
            pointLightColors[0] = glm::vec3(0.0f, 1.0f, 0.0f);
            pointLightColors[1] = glm::vec3(0.0f, 1.0f, 0.0f);
            pointLightColors[2] = glm::vec3(0.0f, 1.0f, 0.0f);
            pointLightColors[3] = glm::vec3(0.2f, 1.0f, 0.2f);
            for(int i=0; i<4; i++){
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].ambient", pointLightColors[i] * 0.1f);
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].diffuse", pointLightColors[i]);
                lightingShader.setVec3("pointLights[" + std::to_string(i) + "].specular", pointLightColors[i]);
        ...
        // point lights static properties
        for(int i=0; i<4; i++){
        ...
        // spotLight property (Camera flashlight)
        ...
        lightingShader.setFloat("spotLight.cutOff", glm::cos(glm::radians(10.5f)));  // slightly narrower flashlight
        ...
        // Only draw lamps if not horror (or draw them anyway)
        glBindVertexArray(lightCubeVAO);
        for (unsigned int i = 0; i < 4; i++)
        {
            ...
            // Optional: You could pass pointLightColors[i] to lightCubeShader to color the light boxes
            glDrawArrays(GL_TRIANGLES, 0, 36);
        }
...
void processInput(GLFWwindow *window)
{
    ...
    // Swap environments
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) environment = 1;
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) environment = 2;
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) environment = 3;
    if (glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS) environment = 4;
}
