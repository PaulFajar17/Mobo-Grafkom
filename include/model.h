...
unsigned int TextureFromFile(const char *path, const string &directory, const aiScene* scene, bool gamma)
{
    ...
    if (scene != nullptr && filename.c_str()[0] == '*')
    {
        // Embedded texture
        ...
        if (textureIndex < scene->mNumTextures) {
            ...
            if (aiTex->mHeight == 0) {
                ...
                // Uncompressed data (ARGB -> RGBA conversion might be needed, but usually it's compressed)
                // For simplicity, we fallback if uncompressed (rare)
                std::cout << "Uncompressed embedded texture not supported!" << std::endl;
            }
        }
    }
    else
